...
export default function EditContentPage({ params }: { params: { id: string } }) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        setFormData({
          ...contentData,
          type: contentData.type.toLowerCase(), // Convert type to lowercase for UI consistency
        ...
        // router.push('/creator/content'); // Removed redirect
      } finally {
        setLoading(false);
      }
    };
  ...
  const getTypeIcon = (type: string) => {
    switch (type.toLowerCase()) {
      case 'story': return <FileText size={20} />;
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p style={{ color: 'var(--color-textSecondary)' }}>
            Refine your work and bring it to perfection.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
            ...
            <div className="space-y-6">
              ...
              <div>
                <label className="block text-sm font-medium mb-2" style={{ color: 'var(--color-textPrimary)' }}>Content Type</label>
                <div className="grid grid-cols-3 gap-4">
                  {['story', 'article', 'book'].map((type) => (
                    <button key={type} type="button" onClick={() => setFormData(prev => ({ ...prev, type }))} className={`flex items-center space-x-2 p-4 rounded-lg border-2 transition-colors ${formData.type === type ? 'shadow-md' : ''}`} style={{ backgroundColor: formData.type === type ? 'var(--color-primary)10' : 'var(--color-backgroundSecondary)', borderColor: formData.type === type ? 'var(--color-primary)' : 'var(--color-border)', color: 'var(--color-textPrimary)' }}>
                      {getTypeIcon(type)}
          ...
          {/* Tags and Settings */}
          <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
            <h2 className="text-xl font-semibold mb-6" style={{ color: 'var(--color-textPrimary)' }}>Tags & Settings</h2>
            <div className="space-y-6">
              <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="flex items-center space-x-2">
                    ...
                    <span className="text-sm" style={{ color: 'var(--color-textPrimary)' }}>Free Content</span>
                  </label>
                </div>
                {!formData.isFree && (
                  <div>
                    <label className="block text-sm font-medium mb-2" style={{ color: 'var(--color-textPrimary)' }}>Subscription Tier Required</label>
          ...
          {/* Linking Section */}
          <div className="p-6 rounded-lg" style={{ backgroundColor: 'var(--color-card)', border: '1px solid var(--color-border)' }}>
            <h2 className="text-xl font-semibold mb-6 flex items-center space-x-2" style={{ color: 'var(--color-textPrimary)' }}><Link2 size={20} /><span>Link Content</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label htmlFor="galleryId" className="block text-sm font-medium mb-2" style={{ color: 'var(--color-textPrimary)' }}>Attach a Gallery</label>
              ...
              <div>
                <label htmlFor="linkedPodcastId" className="block text-sm font-medium mb-2" style={{ color: 'var(--color-textPrimary)' }}>Link to a Podcast</label>
